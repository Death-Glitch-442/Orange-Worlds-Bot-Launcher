...
BEDROCK_API_KEY is included as a template secret (hidden from remixer)
Bot credentials (email/password) should be auto-generated using format bot{random}@automation.com
Accounts auto-create on first login via the Bedrock Passport API
Bot start stagger: 5 seconds between each bot launch to manage resource usage
